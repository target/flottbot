...
func Rules(rules *map[string]models.Rule, bot *models.Bot) {
	...
	if err != nil {
		bot.Log.Fatal().Msgf("Could not parse rules: %s", err)
	}

	// Loop through the rules directory and create a list of rules
	bot.Log.Debug().Msg("Fetching all rule files...")
	fileList := []string{}
	...
	for _, ruleFile := range fileList {
		...
		if err != nil {
			bot.Log.Error().Msgf("Error while reading rule file '%s': %s", ruleFile, err)
		}
